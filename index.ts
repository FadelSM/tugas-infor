...
<body>
  ...
  <script>
    ...
    const nodeLabels = ['Bit-World Servers', 'Cyber ISP Gateway', 'Quantum Router Nexus', 'Digital Soul Device'];
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const particleMaterial = new THREE.PointsMaterial({ color: nodeMaterials[i].color, size: 0.1, opacity: 0.8, transparent: true });
    ...
    function createDataPacket() {
      ...
      const position = dataCurve.getPointAt(Math.random());
      packet.position.copy(position);
      packet.userData.speed = 0.02;
    ...
    function animate() {
      ...
      dataPackets.forEach(packet => {
        ...
        const newPosition = dataCurve.getPointAt(packet.userData.t);
    ...
    renderer.domElement.addEventListener('mousemove', (event) => {
      ...
      const intersects = raycaster.intersectObjects(nodes.filter(node => !!node.geometry));
